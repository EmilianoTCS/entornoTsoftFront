...
const EnviarCorreo = ({
  ...
  const handleContactosChange = (e) => {
    const selectedOptions = Array.from(e.target.selectedOptions).map(
      (option) => {
        const selectedContact = listEDDContactos.find(
          (contact) => contact.correoContacto1 === option.value
        );
        return {
          nomContacto: selectedContact ? selectedContact.nomContacto : "",
          correoContacto: option.value,
          correoContacto2: selectedContact
            ? selectedContact.correoContacto2
            : "",
        };
      }
    );
    ...
    <>
      <Modal show={show} onHide={handleClose} backdrop="static" keyboard={true}>
        <Modal.Header closeButton>
          <Modal.Title>
            Envío correo de evaluaciones a <strong>Referentes</strong>
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <form onSubmit={SendDataEmail}>
            <div className="form-group">
              ...
              <select
                ...
                onChange={({ target }) => {
                  setidEDDProyecto(target.value);
                  obtenerContactos(target.value);
                }}
              >
                <option hidden value="">
                  Desplegar lista
            ...
            <div className="form-group">
              <label htmlFor="contactos">
                Seleccione uno o más contactos (Utilice tecla Ctrl):{" "}
              </label>
              <select
                required
                className="form-control"
              ...
              >
                ...
                {listEDDContactos.map((valor) => (
                  <option value={valor.correoContacto1}>
                    {valor.nomContacto}
                  </option>
                ))}
              </select>
